...
public class ConverterItemProcessor implements ItemProcessor<PassengerPost, ForumPost> {
	...
	public ForumPost process(PassengerPost item) throws Exception {
		...
		return new ForumPost(item.getCommentID(), item.getParentCommentID(), item.getCommentLevel(), item.getUsername(), item.getEmailAddr(), item.getUserType(),
				item.getPostTime(), item.getMessage(), item.getVotes(), item.getAttachmentsCount(), item.getAttachmentLinks());	//Removed Step
	}
}
