...
public class PostItemProcessor implements ItemProcessor<ThreadData, ThreadData> {
	...
	public ThreadData process(ThreadData item) throws Exception {
		...
		ResultSet rs = selectStatement
				.executeQuery("Select * from dbo.EMBARK_CHAT where StepTxt='" + item.getThreadName().replaceAll("'", "''") + "'" + "and ProcessStatusInd = 0");
		Statement stmt = null;
		...
		while (rs.next()) {
			try {
				...
				stmt.executeUpdate("Update dbo.EMBARK_CHAT Set ProcessStatusInd = 1 where StepTxt='" + item.getThreadName().replaceAll("'", "''") + "'");
				dataRows++;
		...
		//stmt.executeUpdate("Update dbo.LiveChat Set ProcessStatusInd = 1 where StepTxt='" + item.getThreadName().replaceAll("'", "''") + "'");
