...
public class JobConfiguration {
    ...
    @Value("file://WST-STSQ-000968/D$/Passenger_Chat/Passenger_Chat_Feed/NM_*.csv")
    //@Value("file://WSP-STSQ-000967/D$/Passenger_Chat/Passenger_Chat_Feed/NM_*.csv")
    //@Value("file://C:/Passenger_Chat/Passenger_Chat_Feed/NM_*.csv")
	...
	public FlatFileItemReader<PassengerPost> passengerPostItemReader() {
		...
		//reader.setResource(new ClassPathResource("/data/NM_Life1.csv"));
		//reader.setResource(new PathResource("//WST-STSQ-000968/D$/Passenger_Chat/Passenger_Chat_Feed/NM_Life Claims Kit Live Chat Transcript_" + sdfDate +".csv"));
		//reader.setResource(new PathResource("//WSP-STSQ-000967/D$/Passenger_Chat/Passenger_Chat_Feed/NM_Life Claims Kit Live Chat Transcript_" + sdfDate +".csv"));
	...
	public ItemWriter<ForumPost> writer(DataSource dataSource, ItemPreparedStatementSetter<ForumPost> setter) {
        ...
        writer.setSql("INSERT INTO dbo.FUEL_CYCLE_CHAT (LastModifiedDtm, LastModifiedByNam, RowCreateDtm, RowCreatedByNam, UsernameTxt, MemberEmailAddrNam, UserTypeTxt, PostDtm, MessageTxt, LikeCountNum, StepTxt, ProcessStatusInd)" +
    			" VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
